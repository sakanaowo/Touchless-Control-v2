...
- Task 4.11 replaced generic README/deployment/monitoring placeholders with the verified Python 3.12 + `uv` workflow, model-path requirements, safe dry-run/dispatch separation, local release and rollback gates, acceptance metrics, JSONL retention guidance, health checks, and incident response.
